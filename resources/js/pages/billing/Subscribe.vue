...
const getDisplayPrice = (plan: Plan): string =>
    trans(`billing.subscribe.prices.${plan.slug}.${isYearly.value ? 'yearly_per_month' : 'monthly'}`);
...
<template>
    ...
    <section class="relative min-h-screen overflow-hidden bg-background">
        ...
        <div class="pointer-events-none absolute top-1/3 -left-32 size-[440px] rounded-full bg-fuchsia-200/30 blur-3xl" />

        <!-- Hero -->
        <div class="relative mx-auto max-w-7xl px-6 pt-14 pb-4 lg:pt-20">
            <div class="mx-auto max-w-3xl space-y-4 text-center">
                <span class="-rotate-1 inline-block rounded-md border-2 border-foreground bg-violet-200 px-3 py-1 text-[11px] font-black uppercase tracking-widest text-foreground shadow-2xs">
                    {{ $t('billing.subscribe.eyebrow') }}
                </span>
                <h1
                    class="text-balance text-3xl font-normal leading-[1.1] tracking-tight text-foreground sm:text-5xl lg:text-6xl"
        ...
        <div class="relative mx-auto max-w-7xl px-4 pb-20">
            ...
            <div class="flex justify-center pt-8 pb-10">
                <div class="relative">
                    ...
                    <span
                        :class="[
                            'pointer-events-none absolute -top-6 -right-12 inline-flex items-center gap-1 whitespace-nowrap rounded-md border-2 border-foreground bg-amber-200 px-2.5 py-1 text-[10px] font-black uppercase tracking-widest text-foreground shadow-xs transition-all sm:top-1/2 sm:right-auto sm:left-full sm:ml-3 sm:-translate-y-1/2 rotate-6 sm:-rotate-3',
                    ...
                    <div class="mb-2">
                        <div class="flex items-baseline gap-1">
                            <span class="relative inline-block overflow-hidden leading-none">
                                ...
                                >
                                    <span :key="getDisplayPrice(plan)" class="block text-5xl font-black tabular-nums text-foreground">
